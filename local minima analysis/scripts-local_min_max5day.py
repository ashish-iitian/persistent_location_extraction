...
def bin_and_return_minima(user_dls,start):
	...
	for x in user_dls:
		...
		if(start <= x[0] < start+ MAX_HOURS*3600):
			for i in range((x[0]-start)//3600,min(end,((x[1]-start)//3600)+1)):
				hrs[i] += 1
    #find hour of greatest mobility
	indx,val = max(enumerate(hrs), key = operator.itemgetter(1))
	if(indx <= 12):
		if(indx >= 4):
			ind = 0
		else:
...
def persistance_dist(user_data):
	""" Returns the distance between two locations estimated to be persistent during that 5-day interval
	"""
	end = max(user_data, key = lambda x: x[1])[1]
	start=end- 86400*5
	ret=bin_and_return_minima(user_data,start)
	if (-1 in ret):
		return None
	lat_long=[]
	for tm in ret:
...
def plot_time_dist_spread(time_diff,loc_diff,time_bin=3600,loc_bin=100,time_max=86400, loc_max=10000):
	...
	(n, bins, patches) = plt.hist(loc_diff, bins = np.arange(0,loc_max,loc_bin))
	...
	(n2, bin2, patch2) = plt.hist(time_diff, bins = np.arange(0,time_max,time_bin))
	...
	plt.xlabel('Time Difference (hours)')
	plt.ylabel('Fraction')
	plt.savefig("/home/akumar13/local_min_time_dist_all5days_loc_bin100.png")
...
def main():
	...
	persistant_locs=data_rdd.map(lambda kv: persistance_dist(kv[1])).filter(lambda x: x is not None).collect()
	...
	short = [x for x in loc_diff if x<2000]
	print len(short)
	print len(loc_diff)
